...
```bash
# Step 1 — fetch real historical tide-gauge data (one-time setup, requires Node.js 18+)
node scripts/fetch_psmsl.js
# outputs data/historical.json (~20 countries from PSMSL / NOAA)
...
Sea level data uses **real tide-gauge records** where available (PSMSL / NOAA CO-OPS, normalized to 1990 baseline), with synthetic fallback for countries without reliable gauge coverage. SSP projection bands are parameterized from IPCC AR6 regional values. No backend required to view the site — run `node scripts/fetch_psmsl.js` once to download real historical data.
...
├── data/
│   └── historical.json # Real tide-gauge data (generated by fetch_psmsl.js; gitignored)
└── scripts/
    └── fetch_psmsl.js  # Fetch real historical data from PSMSL / NOAA CO-OPS
```
...
|---------|--------|------------|
| Bangladesh | PSMSL | Chittagong (1087) |
| Maldives | PSMSL | Malé (1733) |
| Tuvalu | PSMSL | Funafuti (820) |
| Kiribati | PSMSL | Tarawa (1614) |
| Marshall Islands | PSMSL | Kwajalein (851), Majuro (1185) |
| Vietnam | PSMSL | Hon Dau (238) |
| Netherlands | PSMSL | Vlissingen (9), Hoek van Holland (22), IJmuiden (23) |
| Egypt | PSMSL | Alexandria (1) |
| Thailand | PSMSL | Ko Lak (276) |
| India | PSMSL | Mumbai (123), Kochi (49) |
| China | PSMSL | Xiamen (268) |
| Indonesia | PSMSL | Jakarta (185) |
| Philippines | PSMSL | Manila (61) |
| Pakistan | PSMSL | Karachi (20) |
| Nigeria | PSMSL | Lagos II (2079) |
| Mozambique | PSMSL | Beira (937) |
| Japan | PSMSL | Tokyo (1024), Osaka (1316), Aburatsubo (610), Hakata (1318) |
| Taiwan | NOAA | Keelung (1636000), Kaohsiung (1635000) |
